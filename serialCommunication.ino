...
boolean parseState() {
  ...
  state.mode = (acMode_t)(serialRxBuf[12] & 0x0f);
  state.fanSpeed = (fanSpeed_t)(serialRxBuf[13] & 0x7f);

  state.acSetPoint = serialRxBuf[17] >= 100 ? 99 : serialRxBuf[17];
  state.currentTemp = serialRxBuf[26];
...
void handleUart() {
  if (Serial.available()) {
    ...
    if (serialRxBuf[10] == 0xc8) {
      ...
      serialRxBuf[50] == 0xaa &&
      serialRxBuf[51] == 0x1e &&
      serialRxBuf[52] == 0xaC && //Appliance Type
      serialRxBuf[58] == 0x03 &&
      serialRxBuf[59] == 0x64 &&
      serialRxBuf[61] == 0x01 &&
      serialRxBuf[65] == 0x01
...
void writeHeader(byte msgType, byte agreementVersion, byte packetLength) {
  ...
  currentHeader[2] = 0xAC; //ApplianceType
...
void handleStateUpdateRequest(String requestedState, String mode, String fanSpeed, byte acSetPoint) {
  airconditioningState_t newState;

  newState.powerOn = state.powerOn;
  newState.mode = state.mode;
  newState.fanSpeed = state.fanSpeed;
  newState.acSetPoint = state.acSetPoint;
  ...
  if (mode == "automatic") {
    newState.mode = (acMode_t)automatic;
  } else if (mode == "cooling") {
    newState.mode = (acMode_t)cooling;
  } else if (mode == "heating") {
    newState.mode = (acMode_t)heating;
  } else if (mode == "drying") {
    newState.mode = (acMode_t)drying;
  } else if (mode == "fan") {
    newState.mode = (acMode_t)fan;
  }
  ...
  if (acSetPoint && acSetPoint >= 35 && acSetPoint <= 85) {
    newState.acSetPoint = acSetPoint;
  }
  ...
  if ( //Only send if we have updates
    ...
    state.acSetPoint = newState.acSetPoint;
    delay(30);
  }
}
...
void updateSetStatus(boolean powerOn, acMode_t acMode, fanSpeed_t fanSpeed, byte acSetPoint) {
  memset(setStatusCommand, 0, sizeof(setStatusCommand));

  setStatusCommand[0] = 0x48; //Magic
  setStatusCommand[1] = powerOn ? 0x01 : 0x00;
  setStatusCommand[2] = (byte)(acMode & 0x0f);
  setStatusCommand[3] = (byte)fanSpeed;
  setStatusCommand[7] = acSetPoint;
}
