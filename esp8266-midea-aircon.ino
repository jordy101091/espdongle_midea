...
char identifier[24];
#define FIRMWARE_PREFIX "esp8266-midea-airconditioning"
...
char MQTT_TOPIC_AUTOCONF_AIRCON_SENSOR[128];
...
void setup() {
  ...
  snprintf(identifier, sizeof(identifier), "AIRCONDITIONING-%X", ESP.getChipId());
  snprintf(MQTT_TOPIC_AVAILABILITY, 127, "%s/%s/status", FIRMWARE_PREFIX, identifier);
  snprintf(MQTT_TOPIC_STATE, 127, "%s/%s/state", FIRMWARE_PREFIX, identifier);
  snprintf(MQTT_TOPIC_COMMAND, 127, "%s/%s/command", FIRMWARE_PREFIX, identifier);

  snprintf(MQTT_TOPIC_AUTOCONF_AIRCON_SENSOR, 127, "homeassistant/sensor/%s/%s_aircon/config", FIRMWARE_PREFIX, identifier);
...
void publishState() {
  ...
  stateJson["acSetpoint"] = state.acSetPoint;
  stateJson["currentTemp"] = state.currentTemp;
  ...
  switch (state.mode) {
    case automatic:
      stateJson["mode"] = "automatic";
      break;
    case cooling:
      stateJson["mode"] = "cooling";
      break;
    case heating:
      stateJson["mode"] = "heating";
      break;
    case drying:
      stateJson["mode"] = "drying";
      break;
    case fan:
      stateJson["mode"] = "fan";
      break;
  ...
  if (strcmp(topic, MQTT_TOPIC_COMMAND) == 0) {
    ...
    if (!err) {
      handleStateUpdateRequest(
        ...
        commandJson["acSetPoint"].as<byte>()
      );
...
void publishAutoConfig() {
  ...
  device["model"] = "Generic Airconditioning";
  device["name"] = identifier;
  device["sw_version"] = "0.0.1";


  DynamicJsonDocument airconSensorAutoconfPayload(512);

  airconSensorAutoconfPayload["device"] = device.as<JsonObject>();
  airconSensorAutoconfPayload["availability_topic"] = MQTT_TOPIC_AVAILABILITY;
  airconSensorAutoconfPayload["state_topic"] = MQTT_TOPIC_STATE;
  airconSensorAutoconfPayload["name"] = identifier + String(" Airconditioning");
  airconSensorAutoconfPayload["device_class"] = "climate";
  airconSensorAutoconfPayload["unit_of_measurement"] = "%";
  airconSensorAutoconfPayload["value_template"] = "{{value_json.currentTemp}}";
  airconSensorAutoconfPayload["unique_id"] = identifier + String("_aircon");

  serializeJson(airconSensorAutoconfPayload, mqttPayload);
  mqttClient.publish(MQTT_TOPIC_AUTOCONF_AIRCON_SENSOR, mqttPayload, true);
